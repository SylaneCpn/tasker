...
import 'package:tasker/widgets/views/main_page_widgets/daily_tasks_widgets/daily_tasks_view/daily_tasks_list/daily_tasks_list_section.dart';
...
class DoneLastTasksList  extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    ...
    final incommingTasks = dailyTasks.where(
      (t) => t.schedule.next()?.isToday() ?? false,
    );

    final otherTasks = dailyTasks.where(
      (t) => !(t.schedule.next()?.isToday() ?? true),
    );
    return Column(
      children: [
